...
//  designcode
//
//  Created by Developer on 7/3/20.
                        ...
                        ForEach(self.store.courses.indices, id: \.self) { index in
                            GeometryReader { geometry in
                                CourseView(
                                    ...
                                    index: index,
                                    course: self.store.courses[index], bounds: geo)
                                    .offset(y: self.store.courses[index].show ? -geometry.frame(in: .global).minY : 0)
                                    .opacity(self.activeIndex != index && self.active ? 0 : 1)
                                    .scaleEffect(self.activeIndex != index && self.active ? 0.5 : 1)
                                    //moving unactive cards to the right
                                    .offset(x: self.activeIndex != index && self.active ? geo.size.width : 0)
                            }
                            .frame(height: self.horizontalSizeClass == .regular ? 80 : 280)
                            .frame(maxWidth: self.store.courses[index].show ? 712 : getCardWidth(bounds: geo))
...
struct CourseView: View {
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            .padding(30)
            .frame(maxWidth: show ? .infinity : screen.width - 69, maxHeight: show ? .infinity : 280, alignment: .top)
            .offset(y: show ? 460 : 0)
